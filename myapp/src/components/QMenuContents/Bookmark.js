...
const QuestionBox = React.memo(({ question, preview, id, userUid, favorite }) => {
  const [star, setStar] = useState(favorite ? true : false);
  ...
  const handlerStar = async () => {
    const newStarStatus = star === false ? true : false;
    setStar(newStarStatus);
    await updateDoc(doc(db, `users/${userUid}/questions`, id), { favorite: newStarStatus === true });
  };
      ...
      <HStack>
        ...
        <Box pl="5">
          <Pressable onPress={handlerStar}>
            <AntDesign name={star === false ? 'staro' : 'star'} size={24} color="#eab308" />
          </Pressable>
        </Box>
      </HStack>
    </Box>
      ...
      const unsubscribe = onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          const filteredQuestions = querySnapshot.docs
            ...
            .filter(doc => doc.favorite === true);
          setBQuestions(filteredQuestions);
