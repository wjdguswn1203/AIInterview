    ...
    const fetchQuestions = async () => {
      const unsubscribe = onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          const q = query(collection(db, `users/${user.uid}/questions`), where("type", "==", "skill"));
          const querySnapshot = await getDocs(q);
          const fetchedQuestions = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
          setTQuestions(fetchedQuestions);
          setIsq(fetchedQuestions.length > 0);
        }
      });
...
export default TechnicalExample;